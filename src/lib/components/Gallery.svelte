<script lang="ts">
  import { onMount } from 'svelte';
  import type { Snippet } from 'svelte';

  let {
    layout = 'diagonal',
    children,
  }: {
    layout?: 'diagonal' | 'centered';
    children?: Snippet;
  } = $props();

  let lightboxEl: HTMLDivElement | undefined = $state(undefined);
  let contentEl: HTMLDivElement | undefined = $state(undefined);

  onMount(() => {
    if (!lightboxEl || !contentEl) return;

    const backdrop = lightboxEl.querySelector('.gallery-lightbox-backdrop');

    function openLightbox(item: Element) {
      if (!lightboxEl || !contentEl) return;

      const clone = item.cloneNode(true) as HTMLElement;
      clone.classList.remove('collage-item', 'collage-item--custom-rotate');
      clone.removeAttribute('style');
      clone.style.transform = 'none';
      clone.style.width = '100%';
      clone.style.margin = '0';
      clone.style.boxShadow = 'none';
      clone.style.borderRadius = '0';
      clone.style.overflow = 'visible';
      clone.style.cursor = 'default';

      const iframes = clone.querySelectorAll('iframe');
      iframes.forEach((iframe) => {
        const src = iframe.getAttribute('src') || '';
        if (src && !src.includes('autoplay=1')) {
          iframe.setAttribute('src', src + (src.includes('?') ? '&' : '?') + 'autoplay=1');
        }
        iframe.style.width = '100%';
        iframe.style.height = 'auto';
        iframe.style.aspectRatio = '16 / 9';
        iframe.style.position = 'relative';

        const parent = iframe.parentElement;
        if (parent && parent.style.paddingBottom) {
          parent.style.paddingBottom = '0';
          parent.style.position = 'relative';
          iframe.style.position = 'relative';
          iframe.classList.remove('absolute');
        }
      });

      contentEl.innerHTML = '';
      contentEl.appendChild(clone);
      lightboxEl.setAttribute('aria-hidden', 'false');
      document.body.style.overflow = 'hidden';
    }

    function closeLightbox() {
      if (!lightboxEl || !contentEl) return;

      const iframes = contentEl.querySelectorAll('iframe');
      iframes.forEach((iframe) => {
        iframe.setAttribute('src', '');
      });

      lightboxEl.setAttribute('aria-hidden', 'true');
      document.body.style.overflow = '';

      setTimeout(() => {
        if (contentEl) contentEl.innerHTML = '';
      }, 300);
    }

    const collageItems = document.querySelectorAll('.collage-item');
    const handleItemClick = (item: Element) => (e: Event) => {
      if ((e.target as HTMLElement).closest('a')) return;
      e.preventDefault();
      openLightbox(item);
    };

    const itemHandlers: Array<{ el: Element; handler: (e: Event) => void }> = [];
    collageItems.forEach((item) => {
      const handler = handleItemClick(item);
      item.addEventListener('click', handler);
      itemHandlers.push({ el: item, handler });
    });

    const handleBackdropClick = () => closeLightbox();
    backdrop?.addEventListener('click', handleBackdropClick);

    const handleKeydown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && lightboxEl?.getAttribute('aria-hidden') === 'false') {
        closeLightbox();
      }
    };
    document.addEventListener('keydown', handleKeydown);

    // Cleanup all listeners on component destroy
    return () => {
      itemHandlers.forEach(({ el, handler }) => {
        el.removeEventListener('click', handler);
      });
      backdrop?.removeEventListener('click', handleBackdropClick);
      document.removeEventListener('keydown', handleKeydown);
    };
  });
</script>

<div class="gallery-breakout">
  <div class="gallery-collage gallery-collage--{layout}">
    {#if children}
      {@render children()}
    {/if}
  </div>
</div>

<!-- Lightbox overlay (hidden by default, populated by JS) -->
<div
  class="gallery-lightbox"
  bind:this={lightboxEl}
  aria-hidden="true"
>
  <div class="gallery-lightbox-backdrop"></div>
  <div class="gallery-lightbox-content" bind:this={contentEl}></div>
</div>

<style>
  .gallery-breakout {
    width: 100vw;
    margin-left: calc(-50vw + 50%);
    padding: 2rem 1.5rem;
    background:
      radial-gradient(ellipse at 30% 50%, rgba(245, 158, 11, 0.04) 0%, transparent 60%),
      radial-gradient(ellipse at 70% 30%, rgba(179, 136, 255, 0.03) 0%, transparent 50%);
    overflow: visible;
  }

  .gallery-collage {
    max-width: 72rem;
    margin: 0 auto;
    gap: 0.5rem;
  }

  .gallery-collage--diagonal {
    display: grid;
    grid-template-columns: repeat(6, 1fr);
    grid-template-rows: repeat(5, auto);
    grid-template-areas:
      "art   art   art   .     .     ."
      "art   art   art   pixel pixel pixel"
      "art   art   art   pixel pixel pixel"
      ".     .     .     pixel pixel pixel"
      ".     .     .     pixel pixel pixel";
  }

  .gallery-collage--centered {
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    align-items: flex-start;
  }

  /* Lightbox */
  .gallery-lightbox {
    position: fixed;
    inset: 0;
    z-index: 9999;
    display: flex;
    align-items: center;
    justify-content: center;
    opacity: 0;
    pointer-events: none;
    transition: opacity 0.25s ease;
  }

  .gallery-lightbox[aria-hidden="false"] {
    opacity: 1;
    pointer-events: auto;
  }

  .gallery-lightbox-backdrop {
    position: absolute;
    inset: 0;
    background: rgba(0, 0, 0, 0.85);
    cursor: pointer;
  }

  .gallery-lightbox-content {
    position: relative;
    z-index: 1;
    max-width: min(90vw, 1200px);
    max-height: 85vh;
    border-radius: 0.75rem;
    overflow: hidden;
    box-shadow:
      0 20px 60px rgba(0, 0, 0, 0.8),
      0 0 0 3px #1e1a28,
      0 0 0 4px var(--accent, #f59e0b);
    background: #1e1a28;
    transform: scale(0.95);
    transition: transform 0.25s cubic-bezier(0.34, 1.56, 0.64, 1);
  }

  .gallery-lightbox[aria-hidden="false"] .gallery-lightbox-content {
    transform: scale(1);
  }

  /* Content inside lightbox */
  .gallery-lightbox-content :global(img) {
    width: 100%;
    height: auto;
    display: block;
  }

  .gallery-lightbox-content :global(.pixel-box) {
    box-shadow: none;
    background-color: #1e1a28;
  }

  .gallery-lightbox-content :global(p) {
    margin: 0;
    padding: 0.5rem 0.75rem;
    text-align: center;
  }

  /* Iframe in lightbox: make it larger and playable */
  .gallery-lightbox-content :global(iframe) {
    width: 100%;
    aspect-ratio: 16 / 9;
    height: auto;
    display: block;
  }

  .gallery-lightbox-content :global(.relative) {
    position: relative;
    width: 100%;
    padding-bottom: 0;
  }

  @media (max-width: 767px) {
    .gallery-collage--diagonal {
      grid-template-columns: 1fr;
      grid-template-areas:
        "art"
        "pixel";
    }
    .gallery-breakout {
      padding: 1.5rem 1rem;
    }
    .gallery-lightbox-content {
      max-width: 95vw;
    }
  }

  @media (min-width: 1024px) {
    .gallery-breakout {
      padding: 2.5rem 2rem;
    }
  }
</style>
