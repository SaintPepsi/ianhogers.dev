...
<svelte:head>
  <title>Ian Hogers - Gets Stuff DONE</title>
</svelte:head>
...
<section class="mb-12">
  ...
  <div class="grid md:grid-cols-2 gap-6">
    <!-- Dev side -->
    <div class="pixel-box pixel-box-lavender p-6 hover:shadow-lg transition-all group glow-lavender relative">
      ...
      <div class="flex items-center gap-3 mb-4">
        ...
        <h2 class="text-2xl text-purple-300 mb-0 group-hover:opacity-80 transition-opacity">Dev</h2>
      </div>
      <p class="text-xs text-gray-500 font-mono mb-4">code &middot; agents &middot; AI &middot; engineering</p>
      {#if data.devPosts.length > 0}
        <ul class="space-y-2 mb-4">
          {#each data.devPosts as post}
            <li class="flex items-start gap-2">
              <img src="/assets/pixel-art/ui/green_up_arrow_tiny.png" alt="" class="pixel-sprite w-3 h-3 mt-1.5 opacity-60" />
              <a href="/dev/{post.slug}/" class="text-sm text-gray-300 hover:text-purple-300 no-underline transition-colors relative z-20">
                {post.metadata.title}
              </a>
            </li>
          {/each}
      ...
      {/if}
      <span class="text-sm font-mono text-purple-300 no-underline hover:underline inline-flex items-center gap-1">
        all dev posts
    ...
    <!-- Personal side -->
    <div class="pixel-box pixel-box-amber p-6 hover:shadow-lg transition-all group glow-amber relative">
      ...
      <div class="flex items-center gap-3 mb-4">
        ...
        <h2 class="text-2xl text-amber-400 mb-0 group-hover:opacity-80 transition-opacity">Life</h2>
      </div>
      <p class="text-xs text-gray-500 font-mono mb-4">cooking &middot; bunnies &middot; crafts &middot; everything else</p>
      {#if data.personalPosts.length > 0}
        <ul class="space-y-2 mb-4">
          {#each data.personalPosts as post}
            <li class="flex items-start gap-2">
              <img src="/assets/pixel-art/decorative/carrot-small.png" alt="" class="pixel-sprite w-3 h-3 mt-1.5 opacity-60" />
              <a href="/personal/{post.slug}/" class="text-sm text-gray-300 hover:text-amber-400 no-underline transition-colors relative z-20">
                {post.metadata.title}
              </a>
            </li>
          {/each}
      ...
      {:else}
        <p class="text-sm text-gray-600 mb-4 font-mono">Posts incoming...</p>
      {/if}
      <span class="text-sm font-mono text-amber-400 no-underline hover:underline inline-flex items-center gap-1">
        all life posts
        <img src="/assets/pixel-art/ui/green_up_arrow_tiny.png" alt="" class="pixel-sprite w-3 h-3 rotate-90" />
      </span>
    </div>
  ...
  <div class="mt-6">
    <div class="pixel-box pixel-box-maple p-6 hover:shadow-lg transition-all group glow-maple relative">
      ...
      {:else}
        <p class="text-sm text-gray-600 mb-4 font-mono">Posts incoming...</p>
      {/if}
      <span class="text-sm font-mono text-orange-400 no-underline hover:underline inline-flex items-center gap-1">
        all maple posts
        <img src="/assets/pixel-art/ui/green_up_arrow_tiny.png" alt="" class="pixel-sprite w-3 h-3 rotate-90" />
      </span>
    </div>
